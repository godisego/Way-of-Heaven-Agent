...
| <img src="public/avatars/xuan.png" width="56" /> | Host seat · Transformation | **Xuan, the Taoist host** — concludes the conversation | In the three-mentor round he integrates the other two, offering direction and rhythm; **when seated alone he switches to independent Taoist exposition** — wu wei / equality of things / causality |
...
**Three-mentor dialogue (selectable mentors + scoped RAG + citation validation)** — Portraits in the header let you invite or dismiss mentors while keeping at least one. **The default three-mentor round is a linear conclusion**: Old Hu opens on the situation → Li dismantles self-deception → Xuan concludes with direction — not three independent answers to the same question. **A solo seat switches to independent deep answering**: that mentor leaves the ensemble role and answers from their own expertise — for example, Xuan alone shifts from "concluding" to "expounding Taoism directly" (wu wei / equality of things / causality) and no longer says "both make fair points." Bazi and luck-cycle questions can be answered by Old Hu alone. The choice affects the API, retrieval scope, prompts, citations, and voice validation — it is not merely visual. Each mentor may cite only their own library (Li: existentialism/Stoicism; Hu: I Ching, Bazi, and Chinese classics; Xuan: Taoist and Chinese classics; untagged documents are shared). Citations use `[Book title, location]` and are checked against retrieved evidence. A missing or out-of-scope source invalidates **the whole set of citations and triggers a targeted retry**. A deterministic voice validator prevents the mentors from collapsing into one generic AI voice.
