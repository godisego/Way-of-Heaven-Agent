...
- **相关讲义推荐**（2026-08-30 修订）：`findRelatedDocs` 对中文问题按二元词组（bigram）切分并滤掉虚词停用表，拉丁词（embedding/RAG/LoRA 等）整词匹配，匹配讲义标题+简介取分高前三；当前讲义按《标题》提取后排除（slug 与整句上下文都不再误判）。此前的空格分词实现导致自然中文提问永远推荐不出讲义，已修复并实测。
