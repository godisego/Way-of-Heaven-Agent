...
天道智能体当前是本地优先的 RAG 应用：它把用户自己的典籍和笔记转换成可检索知识，再结合问者档生成有出处的三贤回应。

它当前不是完整 Agent。目标 Agent 的工具循环、记忆和验收标准见 `docs/agent-blueprint.md`。
...
检索仍是固定流水线：模型不能自主决定检索什么、是否追加读取原文——这正是 RAG 与 Agent 的边界（工具循环设计见 `docs/agent-loop-design.md`）。分库权属的唯一事实源是 `mentors.ts` 的 traditions 声明。
