...
async function main() {
  ...
  // 场景四 · 库外拒答：语义上与典籍库无交集的问题。
  // 真实嵌入时代"自由意志"这类词能命中存在主义/庄子藏书（原 S4 题因此失效），
  // 换成与库内全部传统（存在主义/斯多葛/易经命理/道家/中华典籍）无语义交集的题目。
  // 库外问题现在默认走联网搜索回退（citations 必为空）；若回退关闭或未命中，
  // 则必须明说「暂未入藏」——两种情况都算无伪造引用。
  console.log("\n场景四 · 库外拒答");
  const s4 = await chat({ question: "怎么做红烧肉才好吃？请给出典籍出处。" });
